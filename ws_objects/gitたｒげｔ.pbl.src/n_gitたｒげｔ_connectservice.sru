$PBExportHeader$n_gitたｒげｔ_connectservice.sru
$PBExportComments$Generated Database Connection Service
forward
global type n_gitたｒげｔ_connectservice from nonvisualobject
end type
end forward

global type n_gitたｒげｔ_connectservice from nonvisualobject
end type
global n_gitたｒげｔ_connectservice n_gitたｒげｔ_connectservice
...
Choose Case is_connectfrom
	...
	Case IS_USE_INIFILE						/*  Populate Database Connection from INI file  */
		string ls_inifile = "C:\てｓｔ\20180125\git1\gitたｒげｔ.ini"
		...
		as_dbparm		= ProfileString ( ls_inifile, "Database", "DBParm", "ConnectString='DSN=PB Demo DB V2017R2;UID=dba;PWD=sql'")
	...
	Case IS_USE_REGISTRY						/*  Populate Database Connection from Registry  */
		...
		If RegistryGet ( ls_registrykey, "DBParm", 		RegString!, as_dbparm ) <> 1 Then
			RegistrySet ( ls_registrykey, "DBParm", 		RegString!, "ConnectString='DSN=PB Demo DB V2017R2;UID=dba;PWD=sql'" )
	...
	Case IS_USE_SCRIPT							/*  Populate Database Connection from Script  */
		...
		as_logpass		= ""
		as_server		= ""
		as_dbparm		= "ConnectString='DSN=PB Demo DB V2017R2;UID=dba;PWD=sql'"
		as_lock			= ""
		as_autocommit	= "false"
...
If SQLCA.SQLCode <> 0 Then
	MessageBox ("データベースから切断できません。", SQLCA.SQLErrText )
End If
...
on n_gitたｒげｔ_connectservice.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_gitたｒげｔ_connectservice.destroy
